...
def letter_counting(b, neg_offset, plus_one, different_letters, num_letters):#counting and adding letters to lists
    ...
    for finding_letters in range(len(b)):#loop for cycling though letters
        ...
        if new_letter == True:#adds new letter to list of letters found and adds 1 to its total times occuring 
            different_letters.append(current_letter)#adds new found letter to list
            num_letters.append(plus_one)#adds one to occurences of new found letter

letter_counting(b, neg_offset, plus_one, different_letters, num_letters)#runs the def to do everything

for print_loop in range(len(different_letters)):#just a loop to print the lists
    print(different_letters[print_loop],": ", num_letters[print_loop], "\n")#the printing


#the origional challenge

string2 = "afgasdasd"#the string
counter = 0#the counting

for i in range(len(string2)):#the loop
    ltr = string2[i]# the letters
    if ltr == "a":#the checking
        counter = counter + 1#the addition

print (counter)#the print
